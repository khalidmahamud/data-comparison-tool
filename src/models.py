...
class GoogleServiceAccount(db.Model):
    """Google Service Account model - stores encrypted service account credentials."""
    __tablename__ = 'google_service_accounts'

    id = db.Column(db.Integer, primary_key=True)
    credentials_json_encrypted = db.Column(db.Text, nullable=False)
    project_id = db.Column(db.String(255))  # Extracted for display
    client_email = db.Column(db.String(255))  # Extracted for display
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
