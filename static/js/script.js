...
function matchSpans(addedSpans, removedSpans, isTemp = false) {
    ...
    addedSpans.forEach(addedSpan => {
        ...
        if (diffId && removedSpansMap.has(diffId)) {
            ...
            const handleAddedOver = () => matchedRemovedSpan.classList.add(hoverClass);
            const handleAddedOut = () => matchedRemovedSpan.classList.remove(hoverClass);
...
function setupDiffHighlighting(row) {
    ...
    const addedSpans = Array.from(colBContent.querySelectorAll('.added[data-diff-id]'));
    const removedSpans = Array.from(colAContent.querySelectorAll('.removed[data-diff-id]'));
...
function makeEditable(element) {
    ...
    textArea.setAttribute('data-original-col-a', colAContent ? colAContent.textContent.trim() : '');
    textArea.setAttribute('data-original-col-a-html', colAContent ? colAContent.innerHTML : '');
    ...
    const previewB = container.querySelector('.diff-preview') || (() => {
    ...
    const previewA = colAContainer ? (colAContainer.querySelector('.col-a-diff-preview') || (() => {
        const div = document.createElement('div');
    ...
    // --- Scroll Sync Setup ---
    const elementsToSync = [colAContent, textArea, previewB, previewA].filter(el => el);
    ...
    const updatePreview = () => {
        ...
        fetch('/preview_diff', {
            ...
            body: `text1=${encodeURIComponent(colAText)}&text2=${encodeURIComponent(colBText)}`
        })
        ...
        .then(data => {
            ...
            previewB.innerHTML = `<div>${data.highlighted_b || ''}</div>`;
             if (previewA) previewA.innerHTML = `<div>${data.highlighted_a || ''}</div>`;
            ...
            // Setup temp highlighting for the preview elements
             const tempAddedSpans = Array.from(previewB.querySelectorAll('.added[data-diff-id]'));
             const tempRemovedSpans = previewA ? Array.from(previewA.querySelectorAll('.removed[data-diff-id]')) : [];
    ...
    textArea.addEventListener('input', function() { updatePreview(); }); // Removed height adjustment
    ...
    setTimeout(() => document.addEventListener('click', handleOutsideClick, { capture: true, once: true }), 300); // Use capture and once for reliability
}
...
function cancelEditing(contentElement) {
    ...
    const colAPreview = colATd ? colATd.querySelector('.col-a-diff-preview') : null;
...
function saveCell(saveBtn) {
    ...
    console.log('Saving cell - Row:', rowIdx, 'Text:', text.substring(0, 50) + '...'); // Debug log
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            contentDiv.innerHTML = data.highlighted_html || text.replace(/\\n/g, '<br>'); // Fallback if html missing
...
function setupFormHandlers() {
    ...
    const createHandler = (action, successCb) => function(e) {
        e.preventDefault();
    ...
    document.querySelectorAll('form[action="/approve"]').forEach(form => {
        form.addEventListener('submit', createHandler('/approve', (button, formData) => {
    ...
    document.querySelectorAll('form[action="/reset_cell"]').forEach(form => {
        form.addEventListener('submit', createHandler('/reset_cell', (button) => {
            button.closest('td').classList.remove('approved', 'yellow-approved', 'red-approved');
    ...
    document.addEventListener('mouseup', e => {
        setTimeout(() => {
            ...
            if (selectedText && !document.querySelector('.editable:focus')) {
                ...
                selBtn.style.left = `${(rect.left + (rect.width / 2) - (selBtn.offsetWidth / 2)) + window.scrollX}px`;
                ...
                if (btnRect.right > window.innerWidth) {
                    selBtn.style.left = `${(window.innerWidth - btnRect.width - 10 + window.scrollX)}px`;
                }
                if (btnRect.left < 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', e => {
        ...
        if (editBtn) {
            makeEditable(editBtn.closest('.cell-container').querySelector('.cell-content'));
            return;
        }
